...
为使您能对 "形象参数 "进行更多操作，我们会自动生成可编辑的配置文件，以便进行自定义。请注意，使用 "构建与测试 "功能时，配置文件不会保存到硬盘中，因此您可以使用 OSC，但在上传形象之前不会看到配置文件。
...
如果找到了，在输入时，给定的输入地址会将信息驱动到形象参数里，而输出地址会发送带有参数值的信息（当参数值变动的时候）。
